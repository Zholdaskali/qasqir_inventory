...
public class CapacityControlService {
    ...
    public void validateCapacityForSizeChange(Nomenclature nomenclature, Double newVolume, Double newHeight, Double newWidth, Double newLength) {
        List<Inventory> inventories = inventoryRepository.findAllByNomenclatureId(nomenclature.getId());
        ...
        for (Inventory inventory : inventories) {
            ...
            if (inventory.getWarehouseContainer() != null) {
                WarehouseContainer container = warehouseContainerRepository.findById(inventory.getWarehouseContainer().getId())
                        .orElseThrow(() -> new DocumentException("Контейнер не найден"));
                if (requiredVolume.compareTo(container.getCapacity().add(calculateVolume(nomenclature, inventory.getQuantity()))) > 0) {
                    throw new DocumentException("Новый объем превышает вместимость контейнера: " + container.getId());
                }
                if ((newHeight != null && newHeight > container.getHeight()) ||
                        (newWidth != null && newWidth > container.getWidth()) ||
            ...
            if (inventory.getWarehouseZone() != null) {
                WarehouseZone zone = warehouseZoneRepository.findById(inventory.getWarehouseZone().getId())
                        .orElseThrow(() -> new DocumentException("Зона не найдена"));
                if (requiredVolume.compareTo(zone.getCapacity().add(calculateVolume(nomenclature, inventory.getQuantity()))) > 0) {
                    throw new DocumentException("Новый объем превышает вместимость зоны: " + zone.getId());
                }
            }
        }
    }
    ...
    public void updateCapacitiesAfterSizeChange(Nomenclature nomenclature, Double newVolume, Double newHeight, Double newWidth, Double newLength) {
        // Сначала проверяем, что новые размеры не превышают емкость контейнеров и зон
        validateCapacityForSizeChange(nomenclature, newVolume, newHeight, newWidth, newLength);

        List<Inventory> inventories = inventoryRepository.findAllByNomenclatureId(nomenclature.getId());

        for (Inventory inventory : inventories) {
            BigDecimal oldVolume = calculateVolume(nomenclature, inventory.getQuantity());
            BigDecimal newRequiredVolume = calculateVolumeWithNewSizes(nomenclature, inventory.getQuantity(), newVolume, newHeight, newWidth, newLength);
            ...
            // Резервируем новый объем, используя новые размеры
            if (inventory.getWarehouseContainer() != null) {
                WarehouseContainer container = warehouseContainerRepository.findById(inventory.getWarehouseContainer().getId())
                        .orElseThrow(() -> new DocumentException("Контейнер не найден"));
                // Важно: использовать calculateVolumeWithNewSizes для точности
                BigDecimal requiredVolumeForContainer = calculateVolumeWithNewSizes(nomenclature, inventory.getQuantity(), newVolume, newHeight, newWidth, newLength);
                reserveContainerCapacity(container, nomenclature, inventory.getQuantity());
            }
            if (inventory.getWarehouseZone() != null) {
                WarehouseZone zone = warehouseZoneRepository.findById(inventory.getWarehouseZone().getId())
                        .orElseThrow(() -> new DocumentException("Зона не найдена"));
                reserveZoneCapacity(zone, newRequiredVolume);
            }
        }
    }
